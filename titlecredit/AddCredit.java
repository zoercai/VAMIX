...
public class AddCredit extends JPanel {
	...
	private JLabel credit = new JLabel("Enter your text ");
	...
	private JLabel duration = new JLabel("Duration (seconds) ");
	...
	private final EmbeddedMediaPlayer previewArea = mediaPlayerComponent
			.getMediaPlayer(); // Maybe have a video player that will play for
	...
	public AddCredit() { // TODO check input is video?
	...
	class addTextVideo extends SwingWorker<Void, Integer> {
		...
		protected Void doInBackground() throws Exception {
			try {
				...
				String createBlank = "avconv -y -filter_complex 'color=black' -t "
						...
						+ frameRate
						+ " -aspect "
						+ videoAspect + " " + _vamixDir + "/blankCredit.mpg";
		...
		protected void done() {
			if (_isCancelled == false) {
				if (_isPreview == true) {
					try {
						String preview = "avplay " + _vamixDir + "/credit.mpg";
	...
	/**
	 * Concatenates credit video to the source video. TODO add logging for edit
	 */
	class concatVideo extends SwingWorker<Void, Integer> {
		...
		protected Void doInBackground() throws Exception {
			...
			if (!sourceField.getText().endsWith(".mpg")) {
				...
				videoLocation = _vamixDir + "/originalCredit.mpg";
			}
